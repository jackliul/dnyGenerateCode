...
public class CodeGenerateFactoryIntelinkOms {

//	private static final Log log = LogFactory.getLog(CodeGenerateFactoryFpxXmsCts.class);
	...
	public static void generateCode(String tableName, String codeName, String keyType){
		...
		String servicePackage = NewPropertiesReaderUtil.getConfigInfo("servicePackage");
		...
		generateService();
		generateMapperClass();
		generateDao(tableName, className);
		generateController();
	}
	...
	private static void generateService() {
        if(hasService){
			...
			root.put("daoPackage", NewPropertiesReaderUtil.getConfigInfo("daoPackage"));
            ...
            String serviceImplPath = NewPropertiesReaderUtil.getConfigInfo("serviceImplPath") + serviceImplName + javaFile; 
	...
	private static void generateMapperClass() {
		...
		hasMapperPath = false; // TODO 暂时不需要生成
        if(hasMapperPath){
            root.put("mapperPackage", NewPropertiesReaderUtil.getConfigInfo("mapperPackage"));
	...
	private static void generateDao(String tableName, String className) {

		if (hasDao) { // TODO
			root.put("daoPackage", NewPropertiesReaderUtil.getConfigInfo("daoPackage"));

			String poName = "I" + root.get("className") + "Dao";
			String poPath = NewPropertiesReaderUtil.getConfigInfo("daoPath") + poName + javaFile;
			FreemarkerEngine.createFileByFTL(cfg, root, "dao.ftl", srcPath, poPath);
		}
	}
	...
	private static void generatePO(String tableName, String className) {
        if(hasPo){
			...
			String lowerPoName = poName.substring(0, 1).toLowerCase() + poName.substring(1, poName.length());
			root.put("poName", poName);
			root.put("lowerPoName", lowerPoName);
	...
	private static void generateQueryVO() {
        if(hasQueryVO){
			...
			String lowerQueryVoName = queryVoName.substring(0, 1).toLowerCase()
					+ queryVoName.substring(1, queryVoName.length());
			root.put("queryVoName", queryVoName);
			root.put("lowerQueryVoName", lowerQueryVoName);
    ...
    private static void generateInfoVO(String tableName, String className) {
        if(hasInfoVO){
            ...
            root.put("feilds", feilds);
			String infoVoName = root.get("className") + "InfoDTO";
			String lowerInfoVoName = infoVoName.substring(0, 1).toLowerCase()
					+ infoVoName.substring(1, infoVoName.length());

			root.put("infoVoName", infoVoName);
			root.put("lowerInfoVoName", lowerInfoVoName);
    ...
    private static String getClassPath() {
		String path = new CodeGenerateFactoryIntelinkOms().getClass().getResource("/").getPath();
		return path;
	}
    
    private CodeGenerateFactoryIntelinkOms(){
    }
    ...
    private static void init(){
        ...
        // 动态生成模块配置
        hasController = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("controllerPath")) ? true : false;
        hasService = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("servicePath")) ? true : false;
        hasMapperPath = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("mapperPath")) ? true : false;
		hasDao = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("daoPath")) ? true : false;
        hasSqlMap = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("sqlMapPath")) ? true : false;
        hasPo = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("poPath")) ? true : false;
        hasQueryVO = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("queryVoPath")) ? true : false;
        hasInfoVO = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("infoVoPath")) ? true : false;
        hasJspPage = StringUtils.isNotEmpty(NewPropertiesReaderUtil.bundlePath.getString("jspPagePath")) ? true : false;
    }
